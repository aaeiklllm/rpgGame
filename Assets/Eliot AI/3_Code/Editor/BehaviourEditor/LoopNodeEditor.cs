...
{
	/// <summary>
	/// Editor extension for LoopNode objects.
	/// </summary>
	...
	public class LoopNodeEditor : UnityEditor.Editor
	{
		...
		private void OnEnable()
		{
			...
			if (_methodData == null)
			{
				if ((target as LoopNode).MethodData.IsEmpty())
				{
					(target as LoopNode).MethodData.GetMethodData(_methodInfo);
					_methodData = (target as LoopNode).MethodData;
				}
				else
				{
					_methodData = (target as LoopNode).MethodData;
		...
		{
			EliotEditorUtility.DrawMiniLabel("Node: Loop", EliotGUISkin.GrayBackground);
			...
			(serializedObject.targetObject as LoopNode).FuncNames = _options;
			...
			(serializedObject.targetObject as LoopNode).MethodData.GetMethodData(_methodInfo);
			(serializedObject.targetObject as LoopNode).MethodData.DrawInspector();
			...
			{
				Undo.RecordObject(target, "Loop Node has been modified");
				serializedObject.ApplyModifiedProperties();
			}
		}
	}
